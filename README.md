...
EvE is a cell segmentation model evaluation tool, built to quantitatively evaluate segmentation
model results without requiring a manually generated segmentation mask for comparison.

## Inputs

EvE takes a path to a single **input directory**, searched recursively — files may be organized
into subfolders in any structure, and file order does not matter.

The directory must contain two types of files:

### 1. Labeled mask files (`.tif`)

Segmentation masks to evaluate, provided as **labeled masks**: each cell's pixels are assigned a
unique integer ID, distinguishing it from every other cell in the image. Background pixels have
the value `0`.
...
<!-- TODO: state how Undersegmented cells (see Classification rules below) are
     reflected in these counts — e.g. counted as 1 TP, or as TP + (n-1) FN. -->
...
| Undersegmented | Two or more ground-truth points fall within a single cell region |
